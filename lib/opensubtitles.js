...
os.prototype.computeHash = function(path, cb) {
	...
	this.on('checksum-ready', function(chksum_part, name) {
		...
		if(t_chksum.length == 3) {
			...
			console.log("1+2 : "+chksum);
			chksum = this.sumHex64bits(chksum, t_chksum[2]);
			console.log("finally "+chksum);
	...
	fs.stat(path, function(err, stat) {
		...
		file_size = stat.size;
		self.emit('checksum-ready', file_size.toString(), 'filesize');
		...
		fs.open(path, 'r', function(err, fd) {
			...
			var t_buffers = [{buf:buf_start, offset:0}, {buf:buf_end, offset:file_size-chunk_size/2}];
			for(var i in t_buffers) {
				fs.read(fd, t_buffers[i].buf, 0, chunk_size, t_buffers[i].offset, function(err, bytesRead, buffer) {
					if(err) return cb(err);
